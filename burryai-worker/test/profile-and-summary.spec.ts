...
describe("Profile and financial summary routes", () => {
  beforeAll(async () => {
    ...
    await env.DB.exec(
      "CREATE TABLE IF NOT EXISTS user_profiles (user_id TEXT PRIMARY KEY NOT NULL, full_name TEXT, country TEXT, student_status TEXT, university TEXT, profession TEXT, skills_json TEXT, other_talents_json TEXT, preferred_work_mode TEXT, city TEXT, state_region TEXT, remote_regions_json TEXT, opportunity_radius_km INTEGER NOT NULL DEFAULT 25, min_hourly_rate REAL NOT NULL DEFAULT 0, onboarding_completed INTEGER NOT NULL DEFAULT 0, resume_summary TEXT, resume_text TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE ON UPDATE CASCADE)"
    )
    await env.DB.exec(
